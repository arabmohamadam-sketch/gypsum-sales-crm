...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    CASE TG_TABLE_NAME
      ...
      ELSE
        v_description := format('ایجاد %s', TG_TABLE_NAME);
    END CASE;
  ...
  ELSIF TG_OP = 'DELETE' THEN
    ...
    CASE TG_TABLE_NAME
      ...
      ELSE
        v_description := format('حذف %s', TG_TABLE_NAME);
    END CASE;
  ...
  ELSE
    ...
    THEN
      ...
      CASE TG_TABLE_NAME
        ...
        ELSE
          v_description := format('حذف نرم %s', TG_TABLE_NAME);
      END CASE;
    ...
    ELSIF TG_TABLE_NAME = 'loading'
      AND (to_jsonb(OLD)->>'confirmed_at')
          IS DISTINCT FROM
          (to_jsonb(NEW)->>'confirmed_at')
    THEN
    ...
    ELSE
      ...
      CASE TG_TABLE_NAME
        ...
        ELSE
          v_description := format('ویرایش %s', TG_TABLE_NAME);
      END CASE;
...
COMMENT ON FUNCTION public.audit_core_business_change() IS
  'Creates readable audit descriptions and stores complete old/new snapshots for core CRM business entities without cross-table field access errors.';
