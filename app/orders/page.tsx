...
function getShippingStatus(
  ...
  const orderWaybills = waybills.filter(
    (waybill) => waybill.order_id === order.id
  );
...
function getShippingStatusLabel(
  ...
  switch (status) {
    ...
    case "issued":
      return "حواله صادر شده";

    case "without_waybill":
      return "بدون حواله";
...
function getShippingStatusClass(
  status: ShippingFilter
): string {
  switch (status) {
    case "loading_confirmed":
...
export default function OrdersPage() {
  ...
  const [shippingFilter, setShippingFilter] =
    useState<ShippingFilter>("all");

  const [
    waybillsByOrderId,
    setWaybillsByOrderId,
  ] = useState<Record<string, Waybill[]>>({});
    ...
    >
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 md:p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_260px_260px]">
          ...
          <div>
            <label
              htmlFor="orders-shipping-status"
              className="mb-2 block text-sm font-bold text-slate-700"
            >
              وضعیت ارسال
            </label>
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="hidden overflow-x-auto md:block">
            <table className="min-w-[1350px] w-full text-right text-sm">
                      ...
                      >
                        ...
                        <td className="px-5 py-5">
                          <div className="flex min-w-[190px] flex-col items-start gap-2">
                            <span
                              className={`inline-flex items-center rounded-full px-3 py-1.5 text-xs font-bold ${getShippingStatusClass(
                                shippingStatus
                              )}`}
                            >
                              {getShippingStatusLabel(
                            ...
                            {canIssueWaybill && (
                              <button
                                ...
                                className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                {waybillLoadingId ===
                                order.id ? (
                                  <>
                            ...
                            {orderWaybills.length >
                              0 && (
                              <Link
                                href={`/waybills?order_id=${order.id}`}
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                      ...
                      <div className="mt-3 rounded-xl bg-slate-50 p-3">
                        ...
                        <div className="mt-2 flex flex-col items-start gap-2">
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-1.5 text-[10px] font-bold ${getShippingStatusClass(
                              shippingStatus
                            )}`}
                          >
                            {getShippingStatusLabel(
                          ...
                          {canIssueWaybill && (
                            <button
                              ...
                              className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              {waybillLoadingId ===
                              order.id ? (
                                <>
